...
bool isValid(int x, int y, vector<vector<int>>& vis, vector<vector<char>>& v, int n, int m){
    if(x<0 or y<0 or x>=n or y>=m) return false;
    if(v[x][y]=='#') return false;
    return true;
}

void dfs(int x, int y, vector<vector<int>>& vis, vector<vector<char>>& v, int n, int m){
    vis[x][y] = 1;
    if(isValid(x+1,y,vis,v,n,m) and !vis[x+1][y]){
        dfs(x+1,y,vis,v,n,m);
    }
    if(isValid(x,y+1,vis,v,n,m) and !vis[x][y+1]){
        dfs(x,y+1,vis,v,n,m);
    }
    if(isValid(x-1,y,vis,v,n,m) and !vis[x-1][y]){
        dfs(x-1,y,vis,v,n,m);
